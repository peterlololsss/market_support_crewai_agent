...
def test_live_assistant_adapter_capabilities_contract():
...
def test_live_assistant_adapter_metrics_contract():
    base_url = _live_adapter_base_url()
    api_key = os.getenv("MARKET_AGENT_LIVE_ADAPTER_API_KEY") or None
...
def test_live_assistant_adapter_batch_contract():
    base_url = _live_adapter_base_url()
    api_key = os.getenv("MARKET_AGENT_LIVE_ADAPTER_API_KEY") or None
    ...
    try:
        ...
        pytest.fail(f"live assistant adapter returned invalid batch contract: {exc}")
...
def test_live_assistant_adapter_preflight_service_scope_contract():
...
def test_live_assistant_adapter_rejects_short_resolve_endpoint():
    base_url = _live_adapter_base_url()
    api_key = os.getenv("MARKET_AGENT_LIVE_ADAPTER_API_KEY") or None
...
def test_live_assistant_adapter_rejects_raw_list_batch_payload():
    base_url = _live_adapter_base_url()
    api_key = os.getenv("MARKET_AGENT_LIVE_ADAPTER_API_KEY") or None
...
def _skip_if_adapter_is_not_running(base_url: str, api_key: str | None) -> None:
    try:
        ...
        pytest.skip(f"live assistant adapter is not running at {base_url}: {exc}")

    if body.get("service") != "assistant-wecom-market-agent-adapter":
        pytest.skip(f"{base_url} is not the assistant market-agent adapter")
