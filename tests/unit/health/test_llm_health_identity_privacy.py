...
def test_health_target_retains_only_opaque_identity() -> None:
    ...
    assert "internal-gateway" not in repr(target).lower()
...
def test_health_state_logs_notifications_and_reports_do_not_expose_raw_target(
    ...
    settings = Settings(
        ...
        llm_base_url="https://PRIVATE-ENDPOINT-CANARY.invalid/v1/",
        llm_api_key="key",
        planner_llm_provider="internal-gateway",
    ...
    class FailingSender:
        def send_text(self, text: str) -> None:
            del text
            raise RuntimeError(
                "internal-gateway private-gpt-model-canary "
    ...
    assert "internal-gateway" not in exposed.lower()
