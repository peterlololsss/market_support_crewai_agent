...
def test_validate_reply_allows_public_urls_but_blocks_internal_locators():
    request = make_request(message="示例官网是什么")
    plan = make_plan(
        request,
    ...
    allowed = validate_reply(
        ReplyResponse(
            response_id="resp-1",
            reply=PrimaryReply(kind="answer", text="官网：http://www.example.com/"),
            actions=[],
        ),
        directive,
        plan,
...
def test_validate_reply_requires_document_evidence_for_knowledge_answer():
    request = make_request(message="介绍一下示例中证1000")
    plan = make_plan(
        request,
    ...
    response = ReplyResponse(
        response_id="resp-1",
        reply=PrimaryReply(kind="answer", text="示例是一家量化私募。"),
        actions=[],
    )
...
def test_validate_reply_allows_knowledge_composer_to_downgrade_to_unable():
    request = make_request(message="介绍一下示例")
    plan = make_plan(
        request,
