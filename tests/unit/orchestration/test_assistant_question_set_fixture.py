"""Integrity guards for the Assistant final-output review question set.
...
``scripts/eval_reply_assistant_question_set.py``.
"""
...
from tests.fixtures.assistant_question_set import (
    LABELS,
    QUESTION_SET,
    Question,
    questions_for_label,
)
from scripts.eval_reply_assistant_question_set import (
    MAX_PARALLEL,
    _bounded_parallel,
    _payload,
...
def test_eval_payload_defaults_to_galaxy_channel(monkeypatch):
    ...
    assert payload["dist_channel_name"] == "示例券商"
    assert payload["group_id"] == "R:000000000000000"
...
def test_eval_can_write_markdown_report(tmp_path):
    ...
    assert "# Assistant Question Set Eval Report" in text
    assert "Q:\n\n请发一下周报" in text
