...
def test_runtime_blocks_explicit_foreign_channel_without_adapter_resolve():
    ...
    assert "示例券商" in response.reply.text
    assert "示例银行" in response.reply.text
    assert response.actions == []
...
def test_runtime_uses_composer_only_for_knowledge_answer():
    ...
    response = asyncio.run(runtime.reply(ReplyRequestShim("介绍一下示例").payload()))
...
def test_runtime_raises_when_composer_returns_invalid_reply_contract():
    ...
    try:
        asyncio.run(runtime.reply(ReplyRequestShim("介绍一下示例").payload()))
    except AgentRuntimeError as exc:
        error = exc
    else:
