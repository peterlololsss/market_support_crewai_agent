...
def test_compile_intent_frame_allows_omitted_or_matching_channel_report_send():
    omitted = make_request(message="发个周报", dist_channel_name="示例银行")
    matching = make_request(message="发个示例银行的周报", dist_channel_name="示例银行")
...
def test_compile_intent_frame_allows_document_evidence_when_policy_enables_mcp():
    request = make_request(message="介绍一下示例中证1000指数增强策略")
