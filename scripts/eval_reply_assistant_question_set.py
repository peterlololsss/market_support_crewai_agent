"""Real-LLM final-output review over the Assistant question set.

Runs each question in ``tests/fixtures/assistant_question_set.py`` through the
...
This is a manual diagnostic (not a CI test): it needs MARKET_AGENT_LLM_API_KEY and an
enabled Document MCP; outbound ``action`` items also need the adapter running to
resolve a real send.

Examples:
    uv run --extra dev python scripts/eval_reply_assistant_question_set.py
    uv run --extra dev python scripts/eval_reply_assistant_question_set.py --parallel 4
    uv run --extra dev python scripts/eval_reply_assistant_question_set.py --live-adapter
    uv run --extra dev python scripts/eval_reply_assistant_question_set.py --label refuse_unsafe
    uv run --extra dev python scripts/eval_reply_assistant_question_set.py --ids 26,87,106
    uv run --extra dev python scripts/eval_reply_assistant_question_set.py --output-md report.md
"""
...
from tests.fixtures.assistant_question_set import (  # noqa: E402
    LABELS,
    QUESTION_SET,
    Question,
)
...
def _payload(question: Question) -> dict:
    conversation_key_base = _env(
        ...
        "wecom:R:000000000000000:1600000000000000",
    )
    return {
        "context_id": f"assistant-eval-{question.id}",
        "conversation_key": f"{conversation_key_base}:q{question.id}",
        "group_id": _env("MARKET_AGENT_EVAL_GROUP_ID", "R:000000000000000"),
        "sender_id": _env("MARKET_AGENT_EVAL_SENDER_ID", "1600000000000000"),
        ...
        or _env("MARKET_AGENT_EVAL_DIST_CHANNEL_NAME", "示例券商"),
...
def _write_markdown_report(
    ...
    *,
    ...
    lines = [
        "# Assistant Question Set Eval Report",
        "",
        ...
        f"Adapter: {'live assistant adapter' if live_adapter else 'fake preflight'}",
...
def main() -> None:
    ...
    parser.add_argument(
        "--parallel",
        type=int,
        default=int(os.getenv("ASSISTANT_EVAL_PARALLEL", "4")),
