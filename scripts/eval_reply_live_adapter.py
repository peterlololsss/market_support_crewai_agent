...
def _assert_adapter_health(base_url: str, api_key: str) -> None:
    ...
    if payload.get("service") != "assistant-wecom-market-agent-adapter":
        raise SystemExit(f"{base_url} is not the assistant market-agent adapter")
...
def main() -> None:
    ...
    os.environ["MARKET_AGENT_LLM_TIMEOUT_SECONDS"] = str(args.llm_timeout_seconds)
    os.environ["MARKET_AGENT_LLM_MAX_TOKENS"] = str(args.llm_max_tokens)
