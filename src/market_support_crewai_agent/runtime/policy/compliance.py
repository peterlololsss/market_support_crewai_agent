...
COMPLIANCE_REASON_SPECS: tuple[ComplianceReasonSpec, ...] = (
    ComplianceReasonSpec(
        ...
        "合规产品/业务请求",
        (
            "Example Capital product/service requests are compliant when they ask for "
    ...
    ComplianceReasonSpec(
        ...
        (
            ...
            "a named Example Capital internal service person are compliant service needs. "
    ...
    ComplianceReasonSpec(
        ...
        (
            ...
            "routing to a named Example Capital service person."
        ),
    ...
    ComplianceReasonSpec(
        ...
        "无关请求",
        (
            "Requests unrelated to Example Capital products, Example Capital company information, "
            "Example Capital customer service, market education, quantitative private-fund "
            "products, or public Example Capital educational content must be refused."
        ),
        "这个问题与示例产品或服务无关，我先不回答。",
    ),
    ComplianceReasonSpec(
        "unknown",
        "无法判断",
...
NON_COMPLIANT_REASON_CODES: tuple[ComplianceReasonCode, ...] = tuple(
    ...
    and spec.reason_code
    not in {"compliant_product_request", "customer_service_request"}
)


def refusal_text_for_reason(reason_code: str) -> str:
    parsed_reason_code = _parse_compliance_reason_code(reason_code)
    spec = _REASON_SPEC_BY_CODE.get(parsed_reason_code)
    if spec is None or not spec.refusal_text:
        return _REASON_SPEC_BY_CODE["unknown"].refusal_text
    return spec.refusal_text


def _parse_compliance_reason_code(reason_code: str) -> ComplianceReasonCode:
    if _is_compliance_reason_code(reason_code):
        return reason_code
    return "unknown"


def _is_compliance_reason_code(reason_code: str) -> TypeGuard[ComplianceReasonCode]:
    return reason_code in _REASON_SPEC_BY_CODE
