...
Rank the most relevant document IDs first; fetched evidence is limited to the IDs you return. Select multiple IDs when the question combines company information with a specifically named product/strategy or FAQ topic. Never return more than max_documents IDs.
For company-wide questions about Example Capital/示例 facts, company introduction/profile/overview, what the company does, team, address, overall/latest/historical/quarterly scale or AUM, or product line, choose the company-introduction candidate rather than a single strategy document unless the user clearly names that strategy.
...
For questions about whether Example Capital has a core strategy, self-operated strategy, 自营盘, 重点策略, or which strategy is core/key, choose the FAQ candidate.
