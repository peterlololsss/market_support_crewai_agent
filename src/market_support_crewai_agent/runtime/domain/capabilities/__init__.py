...
from market_support_crewai_agent.runtime.domain.capabilities.manifests import (
...
CAPABILITY_MANIFEST_REGISTRY = CapabilityRegistry(BUILTIN_CAPABILITY_MANIFESTS)
...
def capability_manifest_prompt_dict() -> list[dict]:
    return [
...
def capability_registry_hash() -> str:
    payload = json.dumps(
        {
            "runtime_capabilities": capability_prompt_dict(),
            "capability_manifests": capability_manifest_prompt_dict(),
        },
        ensure_ascii=False,
        sort_keys=True,
        separators=(",", ":"),
