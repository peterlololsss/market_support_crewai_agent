# CrewAI Reference for AI Coding Assistants

> Relocated from the project-root `AGENTS.md` so the root file can stay focused on
> this repository's operating contract. Use this as reference material when work
> touches CrewAI runtime behavior.
...
> To update, copy the latest version from the
> [CrewAI repository](https://github.com/crewAIInc/crewAI), then preserve this
> relocation note.
...
- **`planning=True` with `planning_config=PlanningConfig(...)`** for reflect-then-act behavior.
  In installed CrewAI 1.14.6, `reasoning` / `max_reasoning_attempts` are deprecated aliases.
...
| `planning` | False | Enable reflect-and-plan before task execution |
| `planning_config` | None | Configure planning effort, attempts, observation, and replanning |
